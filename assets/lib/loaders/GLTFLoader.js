...
class GLTFParser {
	...
	constructor( json = {}, options = {} ) {
		...
		// WKWebView expose parfois createImageBitmap sans savoir restituer
		// correctement les PNG issus d'un bufferView GLB. Le symptôme est un
		// matériau blanc alors que la texture est bien embarquée. Dans Tauri,
		// TextureLoader (HTMLImageElement + URL blob:) est plus fiable.
		let isTauriWebView = false;
		try {

			isTauriWebView = typeof window !== 'undefined' && !! ( window.__TAURI__ || window.parent?.__TAURI__ );
		...
		if ( isTauriWebView || typeof createImageBitmap === 'undefined' || ( isSafari && safariVersion < 17 ) || ( isFirefox && firefoxVersion < 98 ) ) {
